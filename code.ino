...
#include <WiFiClient.h>;                       // importing libraries
#include <ThingSpeak.h>;

const char* ssid = "MONSTER";	// wifi setup
const char* password = "Paswan010@1";

WiFiClient client;

unsigned long myChannelNumber = 1182551;
const char * myWriteAPIKey = "QJTTMOQK9NGYHHOD";	//Things speak
const char * myReadAPIKey = "18VIMAXL6SK8AJDF"; 	   read write 
                                                                                                                     channel API.
int led_1;
int led_2;
int led_3;
...
void loop() {
  ...
  if(val<800)
  {
    ...
    {
      digitalWrite(led1,HIGH);	//program logic 
    }
    if(s2==0)
    {
  ...
  {
    digitalWrite(led1,LOW);
    digitalWrite(led2,LOW);	//Writing data into Thinkspeaks
    digitalWrite(led3,LOW);
  }
